 ...
 *
 * Client to record visits, page views, Goals, Ecommerce activity (product
 * views, add to carts, Ecommerce orders) in a Piwik server.
 ...
 *
 * The PHP Tracking Client provides all features of the Javascript Tracker, such
 * as Ecommerce Tracking, Custom Variable, Event tracking and more.
 ...
 *
 * The following code snippet is an advanced example of how to track a Page View
 * using the Tracking API PHP client.
 *
 ...
 *
 *  // If you want to force the visitor IP, or force the server date time to
 *  // a date in the past,
 *  // it is required to authenticate the Tracking request by calling setTokenAuth
 *  // You can pass the Super User token_auth or any user with 'admin'
 *  // privilege on the website $idSite
 *  $t->setTokenAuth( $token_auth );
 *  $t->setIp( "134.10.22.1" );
 *  $t->setForceVisitDateTime( '2011-04-05 23:55:02' );
 *
 *  // if you wanted to force to record the page view or conversion to a
 *  //specific visitorId
 ...
 *  // In the standard JS API, the content of the <title> tag would be set as
 *  // the page title
 *  $t->doTrackPageView('This is the page title');
 *
 * ### Example: tracking Ecommerce interactions
 *
 * Here is an example showing how to track Ecommerce interactions on your
 * website, using the PHP Tracking API.
 ...
 * (for example, when payment is done with Paypal and user doesn't come back on
 * the website after purchase).
 * For more information about Ecommerce tracking in Piwik, check out the
 * documentation: Tracking Ecommerce in Piwik.
 *
 *      $t = new PiwikTracker( $idSite = 1, 'http://example.org/piwik/');
 *
 ...
 *      // Note that when setting a product category, you can specify an array
 *      // of up to 5 categories to track for this product
 ...
 *
 * To set the visitor IP, or the date and time of the visit, or to force to
 * record the visit (or page, or goal conversion) to a specific Visitor ID,
 * you must call setTokenAuth( $token_auth ). The token_auth must be either the
 * Super User token_auth, or the token_auth of any user with 'admin' permission
 * for the website you are recording data against.
 *
 * @package PiwikTracker
 * @api
 */
...
class PiwikTracker
{
    ...
    /**
     * Ecommerce item page view tracking stores item's metadata in these Custom
     * Variables slots.
     */
    ...
    /**
     * Builds a PiwikTracker object, used to track visits, pages and Goal
     * conversions for a specific website, by using the Piwik Tracking API.
     *
    ...
    /**
     * By default, Piwik expects utf-8 encoded values, for example for the page
     * URL parameter values, Page Title, etc.  It is recommended to only send
     * UTF-8 data to Piwik.  If required though, you can also specify another
     * charset using this function.
     *
    ...
    /**
     * Sets the attribution information to the visit, so that subsequent Goal
     * conversions are properly attributed to the right Referrer URL, timestamp,
     * Campaign Name & Keyword.
     *
     * This must be a JSON encoded string that would typically be fetched from
     * the JS API: piwikTracker.getAttributionInfo() and that you have JSON
     * encoded via JSON2.stringify()
     *
     * If you call enableCookies() then these referral attribution values will
     * be set to the 'ref' first party cookie storing referral information.
     *
     ...
     *
     * If scope is 'visit', it will attempt to read the value set in the first
     * party cookie created by Piwik Tracker ($_COOKIE array).
     *
    ...
    /**
     * Sets the Browser language. Used to guess visitor countries when GeoIP is
     * not enabled
     *
    ...
    /**
     * Sets the user agent, used to detect OS and browser. If this function is
     * not called, the User Agent will default to the current user agent.
     *
    ...
    /**
     * Sets the country of the visitor. If not used, Piwik will try to find the
     * country using either the visitor's IP address or language.
     *
     * Allowed only for Admin/Super User, must be used along with
     * setTokenAuth().
    ...
    /**
     * Sets the region of the visitor. If not used, Piwik may try to find the
     * region using the visitor's IP address (if configured to do so).
     *
     * Allowed only for Admin/Super User, must be used along with
     * setTokenAuth().
     * @param string $region
    ...
    /**
     * Sets the latitude of the visitor. If not used, Piwik may try to find the
     * visitor's latitude using the visitor's IP address (if configured to do
     * so).
     *
     * Allowed only for Admin/Super User, must be used along with
     * setTokenAuth().
    ...
    /**
     * Sets the longitude of the visitor. If not used, Piwik may try to find the
     * visitor's longitude using the visitor's IP address (if configured to do
     * so).
     *
     * Allowed only for Admin/Super User, must be used along with
     * setTokenAuth().
    ...
    /**
     * Enables the bulk request feature. When used, each tracking action is
     * stored until the doBulkTrack method is called. This method will send all
     * tracking data at once.
     */
    ...
    /**
     * Enable Cookie Creation - this will cause a first party VisitorId cookie
     * to be set when the VisitorId is set or reset
     *
    ...
    protected function getCookieName($cookieName) {
        // NOTE: If the cookie name is changed, we must also update the method
        // in piwik.js with the same name.
     ...
     *
     * This should be called before doTrackEcommerceOrder(), or before
     * doTrackEcommerceCartUpdate().
     *
     * This function can be called for all individual products in the cart (or
     * order).
     *
     * SKU parameter is mandatory. Other parameters are optional (set to false
     * if value not known).
     *
     * Ecommerce items added via this function are automatically cleared when
     * doTrackEcommerceOrder() or getUrlTrackEcommerceOrder() is called.
     *
     ...
     *
     * On every Cart update, you must call addEcommerceItem() for each item
     * (product) in the cart, including the items that haven't been updated
     * since the last cart update.
     *
     * Items which were in the previous cart and are not sent in later Cart
    ...
    /**
     * Sends all stored tracking actions at once. Only has an effect if bulk
     * tracking is enabled.
     *
     ...
     *
     * If the Ecommerce order contains items (products), you must call first the
     * addEcommerceItem() for each item in the order.
     *
     * All revenues (grandTotal, subTotal, tax, shipping, discount) will be
     * individually summed and reported in Piwik reports.
     *
    ...
    /**
     * Sets the current page view as an item (product) page view, or an
     * Ecommerce Category page view.
     *
     * This must be called before doTrackPageView() on this product/category
     * page. It will set 3 custom variables of scope "page" with the SKU, Name
     * and Category for this page view.
     * Note: Custom Variables of scope "page" slots 3, 4 and 5 will be used.
     *
     * On a category page, you may set the parameter $category only and set the
     * other parameters to false.
     *
     * Tracking Product/Category page views will allow Piwik to report on
     * Product & Categories conversion rates (Conversion rate = Ecommerce orders
     * containing this product or category / Visits to the product or category).
     *
     ...
     * Calling this function will reinitializes the property ecommerceItems to
     * empty array so items will have to be added again via addEcommerceItem().
     * @ignore
     */
     ...
     * By default Piwik will track requests for the "current datetime" but this
     * function allows you to track visits in the past. All times are in UTC.
     *
    ...
    /**
     * Forces the requests to be recorded for the specified Visitor ID rather
     * than using the heuristics based on IP and other attributes.
     *
     * Allowed only for Admin/Super User, must be used along with
     * setTokenAuth().
     *
     * You may set the Visitor ID based on a user attribute, for example the
     * user email:
     *      $v->setVisitorId( substr(md5( $userEmail ), 0, 16));
     *
     * If not set, the visitor ID will be fetched from the 1st party cookie, or
    ...
    /**
     * If the user initiating the request has the Piwik first party cookie, this
     * function will try and return the ID parsed from this first party cookie
     * (found in $_COOKIE).
     *
     ...
     * party cookie.
     *
     * This function will only work if the user is initiating the current
     * request, and his cookies can be read by PHP from the $_COOKIE array.
     *
    ...
    /**
     * Some Tracking API functionnality requires express authentication, using
     * either the Super User token_auth, or a user with 'admin' access to the
     * website.
     *
     * The following features require access:
     * - force the visitor IP
     * - force the date & time of the tracking requests rather than track for
     *   the current datetime
     * - force Piwik to track the requests to a specific VisitorId rather than
     *   use the standard visitor matching heuristic
     *
    ...
    /**
     * By default, PiwikTracker will read first party cookies from the request
    ...
    /**
     * Returns the maximum number of seconds the tracker will spend waiting for
     * a response from Piwik. Defaults to 600 seconds.
     */
    ...
    /**
     * Sets the maximum number of seconds that the tracker will spend waiting
     * for a response from Piwik.
     *
     * @param int $timeout
     * @throws Exception
     */
    ...
    protected function getRequest($idSite)
    {
        ...
        $url = $this->getBaseUrl() .
            ...
            // XDEBUG_SESSIONS_START and KEY are related to the PHP Debugger,
            // this can be ignored in other languages
            ...
            // Attribution information, so that Goal conversions are attributed
            // to the right referrer or campaign
            // Campaign name
     ...
     *
     * This replicates the piwik.js tracker algorithms for consistency and
     * better accuracy.
     *
     * @param $cookieName
     * @param $cookieValue
